...
class FileBrowser(object):
    ...
    # Constants
    BUTTON_TEXT = 'Browse'                          # display text
    ...
    def __init__(self, form):
        ...
        self.__entry = tk.Entry(self.__frame,       # parent widget
                                textvariable=self.__entryFile,  # current text
                                justify=tk.LEFT)    # start of sting at left
        # connect callback to key press button
        self.__entry.bind(self.ENTRY_BIND, self.__OnFileEntryClick)
        self.__entry.pack(side=tk.LEFT)             # move to left of parent
        self.__entry.pack(fill=tk.X)                # fill horizontal
        self.__entry.pack(expand=tk.YES)            # fill extra window space

        # file dialog button
        self.__button = tk.Button(self.__frame,     # parent widget
